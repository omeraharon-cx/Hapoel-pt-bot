...
import sys
import random
from google import genai  # הספריה החדשה והמעודכנת
...
# --- הגדרת ה-AI של גוגל (המנוע החדש) ---
client = None
if GEMINI_API_KEY:
    try:
        client = genai.Client(api_key=GEMINI_API_KEY)
        print("LOG [AI]: מנוע ה-AI החדש הוגדר בהצלחה")
    except Exception as e:
        print(f"LOG ERROR [AI Init]: {e}")
...
PLAYER_MAP = {
    ...
    "Yonatan Cohen": "יונתן כהן", "Roee David": "רועי דוד", "Mark Costa": "קוסטה", 
    "Shavit Mazal": "שביט מזל", "Andrade Euclides Claye": "קליי", "Chipuoka Songa": "סונגה"
}
...
def get_ai_response(prompt):
    """הפונקציה החדשה והחסינה שמשתמשת ב-SDK המעודכן"""
    if not client: return None
    try:
        print("LOG [AI]: מנסה לייצר תוכן עם ה-SDK החדש...")
    ...
    for art in all_articles:
        ...
        try:
            ...
            if any(k.lower() in content.lower() for k in HAPOEL_KEYS):
                prompt = f"כתוב תקציר של 3 משפטים לאוהדי הפועל פתח תקווה. אם לא עליהם ישירות, החזר 'SKIP'.\n\nכתבה: {content[:2500]}"
                summary = get_ai_response(prompt)
                
                if summary and "SKIP" not in summary.upper():
                    # מניעת כפילויות
                    dup_prompt = f"האם התקציר הבא עוסק באותו נושא בדיוק כמו הקודמים? ענה 'YES' או 'NO'.\n\nקודמים: {recent_sums[-1500:]}\n\nחדש: {summary}"
