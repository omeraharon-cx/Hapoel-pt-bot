...
def get_match_players(fixture_id):
    ...
    try:
        ...
        # לוקחים רק שחקנים ששיחקו (דקות > 0)
        played = [p['player']['name'] for p in players_data if (p['statistics'][0]['games']['minutes'] or 0) > 0]
        return played[:10] # טלגרם מגבילה ל-10 אופציות
    except:
        return ["עומר כץ", "רם לוי", "מתן פלג", "דרור ניר", "רוי נאווי", "איתי שכטר", "שחקן אחר"]
...
def main():
    ...
    if match:
        ...
        if now.hour == 10 and f"match_poster_{today_key}" not in tasks_done:
            prompt = f"Football match day poster: Hapoel Petah Tikva vs {match['opp_name']}, cinematic, blue and white theme."
            img_prompt = get_ai_response(f"Translate to DALL-E prompt: {prompt}")
        ...
        if match['status'] == 'FT' and f"match_end_{today_key}" not in tasks_done:
            ...
            # --- המתנה של 10 דקות לסקר המצטיין ---
            print("⏳ מחכה 10 דקות לשליחת סקר מצטיין...", flush=True)
            time.sleep(600) 
    ...
    if now.weekday() == 2 and now.hour == 12 and f"hist_{today_key}" not in tasks_done:
        hist = get_ai_response("סכם 3 אירועים היסטוריים מרגשים של הפועל פתח תקווה מהשבוע הזה בעבר. עברית, חברי.")
        if hist:
