...
def get_match_players(fixture_id):
    ...
    try:
        ...
        return ["עומר כץ", "רם לוי", "מתן פלג", "דרור ניר", "רוי נאווי", "שחקן אחר"]
...
def main():
    ...
    if match:
        ...
        if now.hour < 12 and f"match_poster_{today_key}" not in tasks_done:
            prompt = f"Create a cinematic football match day poster: Hapoel Petah Tikva vs {match['opp_name']}, blue theme, stadium background."
        ...
        if match['status'] == 'FT' and f"match_end_{today_key}" not in tasks_done:
            ...
            if match['my_score'] > match['opp_score']:
                ...
                res_msg = "תיקו בסיום המשחק של הפועל, ממשיכים הלאה בכל הכח\nיאללה הפועללללל 💙"
            else:
                res_msg = "הפסד כואב של הפועל, לא נורא הפועל להרים את הראש.\nממשיכים האלה חזק, קדימה הפועל מלחמההה 💙"
    ...
    # 5. סריקת RSS רגילה לכתבות
    # (הלוגיקה הקיימת של ה-RSS נשמרת כאן)
    # ... 
